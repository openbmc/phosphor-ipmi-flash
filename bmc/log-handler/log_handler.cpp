...
LogBlobHandler::LogBlobHandler(
  ...
  for (auto& config : configs) {
    ...
    info->actions->onOpen->setCallback([infoP = info.get()](
                                           TriggerableActionInterface& tai) {
      auto data = std::make_shared<std::optional<std::vector<uint8_t>>>();
      do {
        if (tai.status() != ActionStatus::success) {
          fprintf(stderr, "Log file unit failed for %s\n",
                  infoP->blobId.c_str());
          continue;
        }
        if (!infoP->handler->open("", std::ios::in)) {
          fprintf(stderr, "Opening log file failed for %s\n",
                  infoP->blobId.c_str());
          continue;
        }
        auto d = infoP->handler->read(0, std::numeric_limits<uint32_t>::max());
        infoP->handler->close();
        if (!d) {
          fprintf(stderr, "Reading log file failed for %s\n",
                  infoP->blobId.c_str());
          continue;
        }
        *data = std::move(d);
 ...
 */
bool LogBlobHandler::deleteBlob(const std::string&) { return false; }
...
std::vector<uint8_t> LogBlobHandler::read(uint16_t session, uint32_t offset,
  ...
  if (data == nullptr || !*data) {
    throw std::runtime_error("Log data not ready for read");
  }
  if ((*data)->size() < offset) {
    return {};
  }
