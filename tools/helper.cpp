...
{
    return pollStat(session, blob,
                    [](const ipmiblob::StatResponse& resp)
                        -> std::optional<std::pair<bool, uint32_t>> {
                        if (resp.blob_state & ipmiblob::StateFlags::open_read)
                        {
                            return std::make_pair(true, resp.size);
                        }
                        else if (resp.blob_state &
                                 ipmiblob::StateFlags::committing)
                        {
                            return std::nullopt;
                        }
