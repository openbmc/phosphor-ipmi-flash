...
class VersionReadBlobTest : public ::testing::Test
{
  protected:
    ...
    std::vector<uint8_t> vector1{0xDE,0xAD,0xBE,0xEF,0xBA,0xDF,0xEE,0x0D};
};
...
TEST_F(VersionReadBlobTest, VerifyValidRead)
{
     ...
     */
    EXPECT_CALL(*im.at("blob0"), open(_, std::ios::in)).Times(2).WillRepeatedly(Return(true));
...
{
  EXPECT_CALL(*tm.at("blob0"), status())
      .Times(1).WillOnce(Return(ActionStatus::success));
  /* file path gets bound to file_handler on creation so path parameter
   * doesn't actually matter
   */
