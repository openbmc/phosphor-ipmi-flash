...
{
    FirmwareBlobHandler::VerifyCheckResponses result = FirmwareBlobHandler::VerifyCheckResponses::other;
...
bool FirmwareBlobHandler::stat(uint16_t session, struct BlobMeta* meta)
{
     ...
     */
    meta->size = (item->second->imageHandler) ? item->second->imageHandler->getSize() : 0;
    ...
    {
        meta->metadata.push_back(static_cast<std::uint8_t>(checkVerificationState()));
