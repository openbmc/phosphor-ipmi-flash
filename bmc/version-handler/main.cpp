...
std::unique_ptr<blobs::GenericBlobInterface> createHandlerFromJsons(
    const std::unique_ptr<VersionHandlersBuilder>& builder,
    const char* configPath)
{
    std::vector<ipmi_flash::HandlerConfig<ipmi_flash::VersionActionPack>>
        configsFromJson = builder->buildHandlerConfigs(configPath);

    ipmi_flash::VersionInfoMap handlerMap;
    ...
    auto handler =
        ipmi_flash::VersionBlobHandler::create(std::move(handlerMap));
    if (!handler)
    {
...
std::unique_ptr<blobs::GenericBlobInterface> createHandler()
{
    auto builder = std::make_unique<ipmi_flash::VersionHandlersBuilder>();
