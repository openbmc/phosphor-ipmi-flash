...
bool pollVersionStat(std::uint16_t session, ipmiblob::BlobInterface* blob)
{
    using namespace std::chrono_literals;
    static constexpr auto pollingSleep = 5s;
    uint16_t blobState = 0;
    uint16_t mask = blobs::StateFlags::committed |
    ...
    try
    {
        ...
        while (attempts++ < commandAttempts)
        {
            blobState = blob->getStat(session).blob_state & mask;

            switch (blobState)
            {
                case blobs::StateFlags::commit_error:
                    std::fprintf(stderr, "failed\n");
                    exitLoop = true;
                    break;
                case blobs::StateFlags::committing:
                    std::fprintf(stderr, "running\n");
                    break;
                case blobs::StateFlags::committed:
                    exitLoop = true;
                    std::fprintf(stderr, "success\n");
                    break;
                default:
                    std::fprintf(stderr, "wait\n");
            }
            if (exitLoop)
            {
                break;
    ...
    return (blobState == blobs::StateFlags::committed);
}
