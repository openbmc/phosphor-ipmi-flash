...
static void closeIt(struct pci_device_iterator*&& it, const PciAccess*& pci)
{
    pci->pci_iterator_destroy(it);
}
using It = stdplus::Managed<struct pci_device_iterator*,
                            const PciAccess*>::Handle<closeIt>;
    ...
    dataLength(dataLength), pci(pci)
{
    ...
    for (dev = pci->pci_device_next(*it); dev; dev = pci->pci_device_next(*it))
    {
...
void PciAccessBridge::write(const stdplus::span<std::uint8_t>& data)
{
...
void AspeedPciBridge::configure(const ipmi_flash::PciConfigResponse& config)
{
    std::fprintf(stderr, "Received address: 0x%x\n", config.address);

    /* Configure the mmio to point there. */
    std::memcpy(addr + bridge, &config.address, sizeof(config.address));
}
