...
bool FirmwareBlobHandler::close(uint16_t session)
{
     ...
     * it'll perform the install. If it's legacy, it'll do nothing. The
     * verify_image service in the legacy case is responsible for placing the
     * file in the correct staging position.
     */
    return false;
}
